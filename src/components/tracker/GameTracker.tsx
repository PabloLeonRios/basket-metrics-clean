 ...
 *
 * FIX DEMO 2026-03:
 * - Se robusteció el match de sesiones.
 * - Ahora busca por _id / id / sessionId / slug si existieran.
 * - Si no encuentra, muestra IDs disponibles para diagnosticar el desacople.
 */
...
function normalizeSessionIds(session: Partial<TrackerSessionData>) {
  return Array.from(
    new Set(
      [session._id, session.id, session.sessionId, session.slug]
...
function getSessionStorageId(session: {
  ...
  const ids = normalizeSessionIds(session);
  return ids[0] || '';
}

function findDemoSessionById(
  sessions: TrackerSessionData[],
  incomingSessionId: string,
) {
  const normalizedIncoming = String(incomingSessionId || '').trim();

  return (
    sessions.find((item) =>
      normalizeSessionIds(item).includes(normalizedIncoming),
    ) || null
  );
}
...
function getAllDemoEvents(): IGameEvent[] {
  return safeJsonParse<IGameEvent[]>(
...
export default function GameTracker({ sessionId }: { sessionId: string }) {
  ...
  const persistSessionUpdate = useCallback(
    (updateData: Partial<TrackerSessionData>) => {
      ...
      const nextSessions = allSessions.map((item) => {
        const itemIds = normalizeSessionIds(item);
        const incomingId = String(sessionId);

        if (!itemIds.includes(incomingId)) return item;
  ...
  useEffect(() => {
    try {
      ...
      const foundSession = findDemoSessionById(storedSessions, sessionId);

      if (!foundSession) {
        const availableIds = storedSessions
          .flatMap((item) => normalizeSessionIds(item))
          .filter(Boolean)
          .join(' | ');

        console.error('TRACKER SESSION MATCH ERROR', {
          buscado: sessionId,
          disponibles: availableIds,
          sesiones: storedSessions,
        });

        throw new Error(
          `No se encontró la sesión. Buscado: ${sessionId}. Disponibles: ${
            availableIds || 'ninguno'
          }`,
        );
      }
                ...
                team.players
                  ...
                  .map((player) => (
                    <button
                      ...
                      className={`flex-shrink-0 snap-center whitespace-nowrap rounded-full px-3 py-2 text-sm font-bold transition-colors ${
                        selectedPlayer?.id === player._id
